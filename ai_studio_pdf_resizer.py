...
def flatten_and_resize_pdf(input_pdf_path, max_dimensions=(3600, 3600), resolution=72):
    # Extract the base name of the PDF file (without the extension)
    base_name = os.path.splitext(os.path.basename(input_pdf_path))[0]
    
    # Create the output path for the flattened and resized PDF
    output_pdf_path = os.path.join(os.path.dirname(input_pdf_path), f"{base_name}_flattened_resized.pdf")

    # Construct the Ghostscript command to flatten and resize the PDF
    gs_command = [
        "gs",
        ...
        "-dPrinted=true",  # Flatten all layers
    ...
    print(f"Flattened and resized PDF saved at {output_pdf_path}")
...
if __name__ == "__main__":
    ...
    # Flatten and resize the PDF
    flatten_and_resize_pdf(input_pdf_path)
